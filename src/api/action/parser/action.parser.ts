...
class ActionParser {
  ...
  static parse(dataView: ProgressiveDataView, type: number, _length: number): Action | null {
    let action: Action | null = null
    const keyEvent: KeyboardEvent | null = this.parseActionHeader(dataView)
  ...
  private static parseActionHeader(dataView: ProgressiveDataView): KeyboardEvent | null {
  ...
  private static toolBrushAction<T>(
    dataView: ProgressiveDataView,
    type: { new(shapeHandle: number, brush: Brush): T },
  ): T {
    ...
    return new type(shapeHandle, brush)
  }
